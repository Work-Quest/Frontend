  ...
  return (
    <div className="flex flex-row flex-wrap gap-2">
      {tags.map((tag, index) => (
        <span
          key={index}
          className="inline-flex items-center -mt-2 rounded-full bg-[#70BEFF] !text-white px-3 py-0.5 text-xs font-medium font-['Baloo_2'] border border-[#70BEFF]/40"
        >
          {tag}
        </span>
      ))}
    ...
    <section className="h-full rounded-2xl bg-white border border-brown/10 shadow-sm p-5 md:p-6 flex flex-col">
      <div className="flex flex-col sm:flex-row sm:items-start gap-4 flex-1 min-h-0">
        ...
        <div className="flex-1 min-w-0 space-y-3 min-h-0">
          <h2 className="!text-xl !font-bold !text-darkBrown font-['Baloo_2']">Achievements</h2>
          {loading && (
            <p className="text-lightBrown text-sm font-['Baloo_2']">Loading achievements…</p>
          )}
          {isEmpty && (
            <p className="text-lightBrown text-sm font-['Baloo_2']">
      ...
      {!loading && !isEmpty && (
        <Button
          type="button"
          variant="default"
          className="mt-4 pt-5 w-full border-2 border-orange text-orange hover:bg-orange/10 font-['Baloo_2'] font-semibold rounded-xl h-11"
          onClick={onSeeAll}
        >
          See all
        </Button>
