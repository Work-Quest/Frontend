...
import { getCharacterSpriteId } from '@/constants/avatar'
...
export const useBattleLogic = (initial: number | ProjectMemberLike[] = 5) => {
  ...
  const initialCount = useMemo(
    () => (typeof initial === 'number' ? initial : initial.length),
    [initial]
  )
    ...
    if (initialMembers) {
      ...
      setUsers(
        initialMembers.map((m, i) => {
          ...
          return {
            uid: String(m.id),
            charId: getCharacterSpriteId(m.selected_character_id),
  ...
  // Keep battle sprites in sync when project members refetch (e.g. user changed character in profile).
  useEffect(() => {
    if (!initialMembers?.length) return
    setUsers((prev) => {
      if (!prev.length) return prev
      return prev.map((u) => {
        const m = initialMembers.find((x) => String(x.id) === u.uid)
        if (!m) return u
        const nextChar = getCharacterSpriteId(m.selected_character_id)
        const nextName = m.name || m.username || u.name
        if (nextChar === u.charId && nextName === u.name) return u
        return { ...u, charId: nextChar, name: nextName }
      })
    })
  }, [initialMembers])
  ...
  const handleGameAction = async (payload: GameActionPayload) => {
    ...
    if (payload.act === 'BOSS_ATTACK_USER') {
      ...
      const bossConfig =
        ENTITY_CONFIG.bosses[boss.id as keyof typeof ENTITY_CONFIG.bosses]?.actions.attack
    ...
    const userConfig =
      ENTITY_CONFIG.characters[targetUser.charId as keyof typeof ENTITY_CONFIG.characters] ||
      ENTITY_CONFIG.characters['c01']
    ...
    if (payload.act === 'ATTACK') {
      ...
      setUsers((prev) =>
        prev.map((u) => {
          ...
          if (u.slot < attackerOldSlot)
            return { ...u, status: 'shifting_backward', slot: u.slot + 1 }
      ...
      await wait(1000)
      setUsers((prev) =>
        prev.map((u) => (u.status === 'shifting_forward' ? { ...u, status: 'idle' } : u))
      )
    } else if (payload.act === 'REVIVE') {
      setUsers((prev) =>
        prev.map((u) => {
          if (u.uid === payload.userId) return { ...u, status: 'idle', slot: 0 }
          if (u.status !== 'dead' && u.slot >= 0)
            return { ...u, status: 'shifting_backward', slot: u.slot + 1 }
          return u
        })
      )
      await wait(1000)
      setUsers((prev) =>
        prev.map((u) => (u.status === 'shifting_backward' ? { ...u, status: 'idle' } : u))
      )
  ...
  const updateUserCharId = (index: number, charId: string) => {
    setUsers((prev) => {
      const c = [...prev]
      if (c[index]) c[index].charId = charId
      return c
    })
  }
