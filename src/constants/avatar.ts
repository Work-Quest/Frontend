...
export const getColorIdByValue = (colorValue?: string): number => {
  ...
  const index = PRESET_COLORS.findIndex((c) => c.value.toLowerCase() === colorValue.toLowerCase())
...
/** Sprite folder id for battle scene (ENTITY_CONFIG.characters keys: c01…c09). */
export const getCharacterSpriteId = (selectedCharacterId?: number): string => {
  const id =
    selectedCharacterId && selectedCharacterId >= 1 && selectedCharacterId <= 9
      ? selectedCharacterId
      : 1
